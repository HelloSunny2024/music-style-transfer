...
# class GANLoss(nn.Module):
#     def __init__(self, use_lsgan=True, real_label=1.0, fake_label=0.0):
#         super().__init__()
#         self.real_label = real_label  
#         self.fake_label = fake_label
#         self.loss = nn.MSELoss() if use_lsgan else nn.BCELoss()

#     def forward(self, prediction, target_is_real, for_discriminator=True):
#         if target_is_real:
#             target_val = self.real_label if for_discriminator else 1.0
#         else:
#             target_val = self.fake_label

#         target_tensor = torch.full_like(prediction, target_val)
